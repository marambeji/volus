...
import { Home, User, Users, CheckSquare, Calendar, Clock, BarChart3, Bell, X } from 'lucide-react';
...
export default function Navbar({ isOpen, onClose }: NavbarProps) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-40 bg-slate-900/40 backdrop-blur-xs transition-opacity duration-300 md:hidden ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
        onClick={onClose}
      />
