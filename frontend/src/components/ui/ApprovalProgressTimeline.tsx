...
export default function ApprovalProgressTimeline({
  ...
  return (
    <div className={`flex flex-col gap-4 ${className}`}>
      <div className="relative pl-8 space-y-6 before:absolute before:left-[12px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-700">
        {/* Submitted Step */}
        <div className="relative flex items-start gap-3">
          <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-950/60 border-2 border-emerald-500 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xs font-bold shadow-sm">
            <Check size={13} />
          </span>
        ...
        {sortedSteps.map((step) => {
          ...
          return (
            <div key={step.approvalInstanceId || step.stepOrder} className="relative flex items-start gap-3">
              {isApproved && (
                <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">
                  <Check size={13} />
                </span>
              )}
              {isPending && (
                <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-violet-600 text-white flex items-center justify-center text-xs font-bold shadow-sm animate-pulse ring-4 ring-violet-100 dark:ring-violet-950/50">
                  <Clock size={13} />
                </span>
              )}
              {isRejected && (
                <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-red-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">
                  <X size={13} />
                </span>
              )}
              {isSkipped && (
                <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-400 flex items-center justify-center text-xs font-bold">
                  <Ban size={12} />
                </span>
              )}
              {isWaiting && (
                <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 text-slate-400 flex items-center justify-center text-[10px] font-bold">
                  {step.stepOrder}
                </span>
              )}
        ...
        <div className="relative flex items-start gap-3">
          {requestStatus === 'APPROVED' && (
            <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-emerald-600 text-white flex items-center justify-center text-xs font-bold shadow-sm ring-4 ring-emerald-100 dark:ring-emerald-950/50">
              <Check size={14} />
            </span>
          )}
          {requestStatus === 'REJECTED' && (
            <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center text-xs font-bold shadow-sm ring-4 ring-red-100 dark:ring-red-950/50">
              <X size={14} />
            </span>
          )}
          {requestStatus === 'PENDING' && (
            <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-500 flex items-center justify-center text-[10px] font-bold">
              <Clock size={12} />
            </span>
          )}
          {requestStatus === 'CANCELLED' && (
            <span className="absolute -left-[32px] top-0.5 w-6 h-6 rounded-full bg-amber-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">
              <Ban size={12} />
            </span>
          )}
          <div className="pl-1 pt-0.5">
