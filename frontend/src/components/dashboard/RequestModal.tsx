...
export default function RequestModal({ isOpen, onClose }: RequestModalProps) {
  ...
  // Fetch Configuration & Leave Types & My Requests & Public Holidays
  useEffect(() => {
    if (isOpen) {
      setLoading(true);
      setErrorMsg('');
      const fetchConfig = async () => {
        try {
          ...
          const [conf, allLeaveTypes, existingRequests, allHolidays, allEmployees] = await Promise.all([
          ...
          // Find current employee record if available
          const me = (allEmployees || []).find((e: any) => e.id === currentUserId);

          const empCountryId = conf?.countryId || me?.countryId;
          const empCountryCode = conf?.countryCode || me?.countryCode;
          const empCountryName = conf?.countryName || me?.country;

          const filteredHolidays = (allHolidays || []).filter((h: any) => {
            if (empCountryId && h.countryId === empCountryId) return true;
            if (empCountryCode && h.countryCode?.toUpperCase() === empCountryCode?.toUpperCase()) return true;
            if (empCountryName && h.country?.toLowerCase() === empCountryName?.toLowerCase()) return true;
            if (h.country?.toLowerCase() === 'global' || h.countryCode?.toUpperCase() === 'GLOBAL') return true;

            // Fallback for Lebanon if no specific country metadata was returned
            if (!empCountryId && !empCountryCode && !empCountryName) {
              return h.countryCode?.toUpperCase() === 'LB' || h.country?.toLowerCase() === 'lebanon' || h.country?.toLowerCase() === 'global';
            }
            return false;
          });
          ...
          // Maternity/Paternity leave are sex-specific — hide the one that doesn't apply.
          const genderFilteredBalances = combinedBalances.filter((b: any) => {
            const code = b.code?.toLowerCase();
            if (code === 'maternity' && me?.gender === 'MALE') return false;
            if (code === 'paternity' && me?.gender === 'FEMALE') return false;
            return true;
          });

          const finalConfig = { ...(conf || {}), balances: genderFilteredBalances };
          setConfig(finalConfig);

          if (genderFilteredBalances.length > 0) {
            const defaultType = genderFilteredBalances.find((t: any) => t.code?.toUpperCase() === 'ANNUAL') || genderFilteredBalances[0];
