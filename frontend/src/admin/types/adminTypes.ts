...
export interface LeaveQuota {
  ...
  approvalWorkflowId?: string; // Required: each leave type must have its own workflow
...
export interface CountryPolicy {
  ...
  leaveQuotas: LeaveQuota[]; // Each quota now has its own approvalWorkflowId
  createdAt: string;
  updatedAt: string;
...
// ─── Department ──────────────────────────────────────────────────────────────
// Backed by the real `/departments` table — see services/departmentsApi.ts
// for the shape returned by the backend (DepartmentItem).
