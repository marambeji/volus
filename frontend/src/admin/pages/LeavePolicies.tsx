...
const ALL_STANDARD_LEAVE_TYPES: Array<{ key: string; label: string }> = [
  { key: 'annual', label: 'Annual Leave' },
  { key: 'sick', label: 'Sick Leave' },
  { key: 'bereavement', label: 'Bereavement Leave' },
  { key: 'wedding', label: 'Wedding Leave' },
  { key: 'paternity', label: 'Paternity Leave' },
  { key: 'maternity', label: 'Maternity Leave' },
  { key: 'public_holiday', label: 'Public Holiday' },
  { key: 'compensation', label: 'Compensation Leave' },
  { key: 'unpaid', label: 'Unpaid Leave' },
  { key: 'overtime', label: 'Overtime Leave' },
];

const defaultQuotaForType = (typeKey: string, typeId?: string, defaultWorkflowId?: string): LeaveQuota => {
  ...
  return {
    ...
    approvalWorkflowId: defaultWorkflowId,
    entitlementDays:
      keyLower === 'annual'
        ? 25
        : keyLower === 'sick'
        ...
        : keyLower === 'wedding'
        ? 3
        : keyLower === 'bereavement'
        ? 3
        : keyLower === 'paternity'
        ? 5
        : keyLower === 'maternity'
        ? 70
        : keyLower === 'public_holiday'
        ? 10
...
export default function LeavePolicies() {
  ...
  const activeLeaveTypes = (() => {
    const list: Array<{ id: string; key: string; label: string }> = [];
    const seenKeys = new Set<string>();

    // Add types from DB first
    leaveTypes.forEach((lt) => {
      if (!lt.key.toLowerCase().startsWith('e2e') && !lt.label.toLowerCase().startsWith('e2e')) {
        list.push({ id: lt.id, key: lt.key, label: lt.label });
        seenKeys.add(lt.key.toLowerCase());
      }
    });

    // Merge standard types that might not be in DB yet
    ALL_STANDARD_LEAVE_TYPES.forEach((std) => {
      if (!seenKeys.has(std.key.toLowerCase())) {
        list.push({ id: `lt-${std.key}`, key: std.key, label: std.label });
        seenKeys.add(std.key.toLowerCase());
      }
    });
    ...
    activeLeaveTypes.forEach((lt) => {
      ...
      if (existing) {
        map[keyOrId] = {
          ...
          leaveType: lt.key,
          approvalWorkflowId: existing.approvalWorkflowId || '',
        };
      } else {
  ...
  function handleCurrentQuotaChange<K extends keyof LeaveQuota>(key: K, val: LeaveQuota[K]) {
    ...
    setQuotasByTypeId((prev) => {
      ...
      // Auto-fill other unassigned quotas so the user doesn't have to manually select on all 10 tabs
      if (key === 'approvalWorkflowId' && val) {
        Object.keys(updated).forEach((id) => {
          if (!updated[id].approvalWorkflowId) {
            updated[id] = { ...updated[id], approvalWorkflowId: val as string };
          }
        });
      }
      return updated;
  ...
  async function handleQuotaWorkflowLevelChange(workflowId: string, levelIdx: number, field: string, value: any) {
    const targetWf = workflows.find((w) => w.id === workflowId);
    if (!targetWf) return;

    const nextLevels = [...targetWf.levels];
    const prev = nextLevels[levelIdx] || { type: 'manager' };

    if (field === 'type') {
      nextLevels[levelIdx] = {
        ...prev,
        type: value,
        departmentId: value === 'specific_employee' ? prev.departmentId : undefined,
        specificApproverEmployeeId: value === 'specific_employee' ? prev.specificApproverEmployeeId : undefined,
      };
      ...
      nextLevels[levelIdx] = {
        ...
        specificEmployeeEmail: selectedEmp?.email || undefined,
      };
    }

    const updatedWf: ApprovalConfiguration = {
      ...targetWf,
      levels: nextLevels,
    };
    ...
    try {
      if (workflowId && workflowId.length > 20) {
        await updateApprovalWorkflow(workflowId, updatedWf);
      }
    } catch (err) {
      console.warn('Failed to auto-update workflow step:', err);
    }
  }
  ...
  function validate(): boolean {
    ...
    // Validate that each leave type has an approval workflow
    Object.keys(quotasByTypeId).forEach((typeId) => {
      const quota = quotasByTypeId[typeId];
      if (!quota.approvalWorkflowId) {
        const ltItem = activeLeaveTypes.find((t) => t.id === typeId || t.key === typeId);
        const displayLabel = ltItem?.label || quota.leaveType;
        newErrors[`workflow_${typeId}`] = `${displayLabel} requires an approval workflow`;
        if (!newErrors.form) {
          newErrors.form = 'All leave types must have an approval workflow assigned.';
        }
      }
    });
              ...
              <div className="flex gap-4 flex-1 overflow-hidden min-h-96">
                ...
                {currentQuota ? (
                  <div className="w-2/3 space-y-4 overflow-y-auto pl-1 pr-2">
                    ...
                    {/* Approval Workflow Selector - Required */}
                    <div>
                      <label className="block text-[9px] font-bold text-slate-400 uppercase mb-1">
                        Approval Workflow *
                      </label>
                      <select
                        value={currentQuota.approvalWorkflowId || ''}
                        onChange={(e) => handleCurrentQuotaChange('approvalWorkflowId', e.target.value)}
                        className={`w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-700 border ${
                          errors[`workflow_${selectedLeaveTypeId}`] ? 'border-red-500' : 'border-slate-200 dark:border-slate-600'
                      ...
                      >
                        ...
                        {workflows.map((w) => {
                          const lvlCount = w.levelsCount || w.levels?.length || 1;
                          return (
                            <option key={w.id} value={w.id}>
                              {w.name} ({lvlCount} {lvlCount === 1 ? 'level' : 'levels'})
                            </option>
                          );
                        })}
                      </select>
                      {errors[`workflow_${selectedLeaveTypeId}`] && (
                        <p className="text-red-500 text-[9px] mt-1 flex items-center gap-0.5">
                          <AlertCircle size={8} /> {errors[`workflow_${selectedLeaveTypeId}`]}
                        </p>
                      )}
                    </div>
                    ...
                    {(() => {
                      const selectedWf = workflows.find((w) => w.id === currentQuota.approvalWorkflowId);
                      if (!selectedWf || !selectedWf.levels || selectedWf.levels.length === 0) return null;
                      ...
                      return (
                        <div className="space-y-3 pt-1">
                          <h5 className="font-bold text-[10px] text-slate-400 uppercase tracking-wide">
                            Approval Sequence Configuration
                          </h5>
                          {selectedWf.levels.map((lvl, index) => {
                            const isSpecific = lvl.type === 'specific_employee' || (lvl.type as string) === 'SPECIFIC_PERSON';
                            const deptVal = (lvl.departmentId || '').trim();
                            const selectedDept = departments.find(
                              (d) => d.id === deptVal || d.name.toLowerCase() === deptVal.toLowerCase()
                            );
                            const targetDeptName = (selectedDept?.name || deptVal).toLowerCase();

                            const availableEmps = employees
                              .filter((e) => {
                                const isAct = e.status === 'ACTIVE' || (e.status as string) === 'active' || !e.status;
                                if (!isAct) return false;
                                if (!deptVal) return false;
                                const matchesDeptName =
                                  e.department && e.department.toLowerCase().trim() === targetDeptName;
                                const matchesDiv = e.divisionId === deptVal;
                                return matchesDeptName || matchesDiv;
                              })
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-md bg-violet-50 dark:bg-violet-950/40 text-violet-600 dark:text-violet-400 border border-violet-100 dark:border-violet-900/40">
                                    {approverTypeLabel}
                                  </span>
                                </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-3 pt-1">
                                    <div>
                                      ...
                                      <select
                                        ...
                                        onChange={(e) =>
                                          handleQuotaWorkflowLevelChange(selectedWf.id, index, 'departmentId', e.target.value)
                                        }
                                    ...
                                    <div>
                                      ...
                                      <select
                                        ...
                                        disabled={!lvl.departmentId}
                                        onChange={(e) =>
                                          handleQuotaWorkflowLevelChange(
                                            selectedWf.id,
                                            index,
                                        ...
                                        className="w-full px-3 py-2 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-violet-500 disabled:opacity-50 disabled:cursor-not-allowed"
                                      >
                                        {!lvl.departmentId ? (
