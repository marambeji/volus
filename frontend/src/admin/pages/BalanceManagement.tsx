...
export default function BalanceManagement() {
  ...
  // Maternity/Paternity are sex-specific — hide the one that doesn't apply,
  // same rule as the employee-facing Request Time Off form.
  const selectedEmployee = state.employees.find(e => String(e.id) === selectedEmpId);
  const allBalances: BalanceItem[] = (balanceData?.balances ?? []).filter(b => {
    const code = b.code?.toLowerCase();
    if (code === 'maternity' && selectedEmployee?.gender === 'male') return false;
    if (code === 'paternity' && selectedEmployee?.gender === 'female') return false;
    return true;
  });
