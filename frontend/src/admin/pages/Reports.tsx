...
import { MonthlyUsageBarChart, BreakdownChart, ActiveFilterChip } from '../../components/reports/reportCharts';
...
function currentAdminName(): string {
  try {
    return JSON.parse(localStorage.getItem('currentUser') || '{}').name || 'HR Admin';
  } catch {
    return 'HR Admin';
  }
}
...
export default function Reports() {
  ...
  const [selectedChartCountry, setSelectedChartCountry] = useState<string | null>(null);
  const [selectedChartDept, setSelectedChartDept] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setSelectedChartCountry(null);
    setSelectedChartDept(null);
  }, [activeReport]);

  const filters = { dateFrom, dateTo, leaveTypeId, department, country, managerId, status };

  // Always load requests + balances + overlaps together (same pattern as the
  // manager Team Reports page) so the Smart PDF Report can be generated from
  // any tab without an extra on-demand fetch.
  useEffect(() => {
    const controller = new AbortController();
    setLoading(true);
    setApiError(null);
    Promise.all([
      getRequestsReport(filters, controller.signal).then(setRequestsData),
      getBalancesReport(filters, controller.signal).then(setBalancesData),
      getOverlapsReport(filters, controller.signal).then(setOverlapsData),
    ])
      .catch((err) => {
        if (err instanceof Error && err.name === 'AbortError') return;
        setApiError(err instanceof Error ? err.message : 'Failed to load report data');
      })
      .finally(() => setLoading(false));
    return () => controller.abort();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dateFrom, dateTo, leaveTypeId, department, country, managerId, status]);
  ...
  const searchedBalances = balancesData
    ...
    .filter((b) => !selectedChartDept || b.department === selectedChartDept);
  ...
  const getEmployeesForType = (type: string) => {
    const byEmployee = new Map<string, { totalRequests: number; totalDays: number }>();
    requestsData
      .filter((r) => r.leaveTypeName === type)
      .forEach((r) => {
        const entry = byEmployee.get(r.employeeName) ?? { totalRequests: 0, totalDays: 0 };
        entry.totalRequests += 1;
        if (r.status === 'APPROVED') entry.totalDays += r.durationDays;
        byEmployee.set(r.employeeName, entry);
      });
    return Array.from(byEmployee.entries()).map(([employeeName, v]) => ({ employeeName, ...v }));
  };
        ...
        <>
          ...
          {activeReport === 'requests' && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-sm">
              ...
              <p className="text-xs text-slate-400 mb-4">Most-used periods — summer months highlighted. Click a bar to filter the list below</p>
          ...
          {activeReport === 'balances' && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-sm">
              ...
              <p className="text-xs text-slate-400 mb-4">Click a bar to filter the list below</p>
              <BreakdownChart
                data={daysUsedByDepartment}
                label="Days used"
                onBarClick={(name) => setSelectedChartDept((prev) => (prev === name ? null : name))}
                activeKey={selectedChartDept}
              />
            </div>
          )}
          {activeReport === 'countries' && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-sm">
              ...
              <p className="text-xs text-slate-400 mb-4">Click a bar to filter the list below</p>
              <BreakdownChart
                data={daysUsedByCountryFromRequests}
                label="Days used"
                onBarClick={(name) => setSelectedChartCountry((prev) => (prev === name ? null : name))}
                activeKey={selectedChartCountry}
              />
            </div>
          )}
          {activeReport === 'departments' && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-sm">
              ...
              <p className="text-xs text-slate-400 mb-4">Click a bar to filter the list below</p>
              <BreakdownChart
                data={daysUsedByDepartmentFromRequests}
                label="Days used"
                onBarClick={(name) => setSelectedChartDept((prev) => (prev === name ? null : name))}
                activeKey={selectedChartDept}
              />
          ...
          {activeReport === 'types' && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-sm">
              <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-1">Requests by Leave Type</h3>
              <p className="text-xs text-slate-400 mb-4">Click a bar to filter the list below</p>
              <BreakdownChart
                data={getTypesReport().map((t) => ({ name: t.type, value: t.totalRequests }))}
                label="Requests"
                onBarClick={(name) => setSelectedType((prev) => (prev === name ? null : name))}
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col">
                ...
                <>
                  {selectedChartDept && (
                    <div className="px-4 pt-4">
                      <ActiveFilterChip label={selectedChartDept} onClear={() => setSelectedChartDept(null)} />
                    </div>
                  )}
              ...
              {activeReport === 'types' && (
                <div className="overflow-x-auto">
                  {selectedType && (
                    <div className="px-4 pt-4">
                      <ActiveFilterChip label={selectedType} onClear={() => setSelectedType(null)} />
                    </div>
                  )}
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50 text-xs">
                      {getTypesReport().filter((t) => !selectedType || t.type === selectedType).map((t) => (
                  ...
                  {selectedType && (
                    <table className="w-full text-left text-sm border-t border-slate-100 dark:border-slate-700">
                      ...
                      <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50 text-xs">
                        {getEmployeesForType(selectedType).map((e) => (
                          <tr key={e.employeeName} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30">
                            <td className="py-3.5 px-4 font-bold text-slate-800 dark:text-white">{e.employeeName}</td>
                            <td className="py-3.5 px-4 text-center font-semibold text-slate-600 dark:text-slate-300">{e.totalRequests}</td>
                            <td className="py-3.5 px-4 text-center font-extrabold text-slate-800 dark:text-white">{e.totalDays} days</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {activeReport === 'countries' && (
                <div className="overflow-x-auto">
                  {selectedChartCountry && (
                    <div className="px-4 pt-4">
                      <ActiveFilterChip label={selectedChartCountry} onClear={() => setSelectedChartCountry(null)} />
                    </div>
                  )}
                  <table className="w-full text-left text-sm">
                    <thead>
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50 text-xs">
                      {getCountryReport().filter((c) => !selectedChartCountry || c.country === selectedChartCountry).map((c) => (
              ...
              {activeReport === 'departments' && (
                <div className="overflow-x-auto">
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50 text-xs">
                      {getDepartmentReport().filter((d) => !selectedChartDept || d.name === selectedChartDept).map((d) => (
