...
import { Check, X, Download, Calendar, History, Trash2 } from 'lucide-react';
...
export default function LeaveRequests() {
  ...
  const leaveTypes = [...new Map(requests.map(r => [r.leaveTypeId, r.leaveTypeName ?? r.leaveTypeId])).entries()];
  ...
  const filtered = requests.filter(req => {
    ...
    const matchesStatus = !filterStatus || req.currentStatus?.toLowerCase() === filterStatus;
    ...
    const matchesStartDate = !filterStartDate || req.startDate?.slice(0, 10) >= filterStartDate;
    const matchesEndDate = !filterEndDate || req.endDate?.slice(0, 10) <= filterEndDate;
  ...
  async function handleDelete(id: string, reason: string) {
    if (!reason.trim()) return;
    try {
      await hrDeleteLeaveRequest(id, reason);
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-6 relative">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <SelectFilter label="Status"     value={filterStatus} onChange={setFilterStatus} options={[{ label:'Pending', value:'pending' }, { label:'Approved', value:'approved' }, { label:'Declined', value:'declined' }, { label:'Cancelled', value:'cancelled' }, { label:'Deleted by HR', value:'deleted_by_hr' }]} />
        <SelectFilter label="Leave Type" value={filterType}   onChange={setFilterType}   options={leaveTypes.map(([id, name]) => ({ label: name, value: id }))} />
                        ...
                        {deleteId === req.requestId ? (
                          <div className="flex items-center gap-1.5">
                            <input
                              ...
                              onKeyDown={e => {
                                if (e.key === 'Enter' && deleteReason.trim()) handleDelete(req.requestId, deleteReason);
                                if (e.key === 'Escape') setDeleteId(null);
                              }}
                              placeholder="Reason for deletion..."
                              className="flex-1 min-w-0 px-2 py-1.5 text-xs bg-slate-50 dark:bg-slate-700 border border-red-200 dark:border-red-900 rounded-lg text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500"
                            />
                            <button disabled={!deleteReason.trim()} onClick={() => handleDelete(req.requestId, deleteReason)} className="p-1.5 rounded-lg text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 cursor-pointer" title="Confirm Delete"><Check size={14}/></button>
                            <button onClick={() => setDeleteId(null)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer" title="Cancel"><X size={14}/></button>
                          </div>
                        ) : rejectId === req.requestId ? (
                          <div className="flex items-center gap-1.5">
                            <input
                              autoFocus
                              type="text"
                              ...
                              onKeyDown={e => {
                                if (e.key === 'Enter' && rejectComment.trim()) handleReject(req.requestId, rejectComment);
                                if (e.key === 'Escape') setRejectId(null);
                              }}
                              placeholder="Reason for rejection..."
                              className="flex-1 min-w-0 px-2 py-1.5 text-xs bg-slate-50 dark:bg-slate-700 border border-red-200 dark:border-red-900 rounded-lg text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500"
                            />
                            <button disabled={!rejectComment.trim()} onClick={() => handleReject(req.requestId, rejectComment)} className="p-1.5 rounded-lg text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 cursor-pointer" title="Confirm Reject"><Check size={14}/></button>
                            <button onClick={() => setRejectId(null)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer" title="Cancel"><X size={14}/></button>
                          </div>
                        ) : (
