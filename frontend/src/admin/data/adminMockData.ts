import type { AdminEmployee, CountryPolicy, AdminLeaveRequest, AuditLogEntry, AdminNotification } from '../types/adminTypes';
...
// (Add Employee, Approval Workflow's Specific Person picker, ...). Backed by
// the real `/departments` table now (see services/departmentsApi.ts) — this
// static fallback just seeds the dropdown before that data has loaded.
export const CANONICAL_DEPARTMENT_NAMES = [
  'Engineering', 'Marketing', 'Human Resources', 'Finance', 'Sales',
  'Operations', 'Legal', 'Design', 'Executive',
];
