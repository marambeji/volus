...
import type { AdminState, AuditAction, AdminEmployee, CountryPolicy, AuditLogEntry, ApprovalConfiguration } from '../types/adminTypes';
import type { LeaveTypeKey } from '../../types';
import {
  adminLeaveLedger,
  adminHolidays, adminAuditLog, adminNotifications
...
const initialState: AdminState = {
  ...
  policies: [],          // Populated from the backend; start empty to avoid stale mock data
...
type Action =
  ...
  | { type: 'SET_POLICIES'; payload: CountryPolicy[] }
...
export function AdminProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const [backendEmps, backendPolicies] = await Promise.all([
          getEmployees(),
          getPolicies().catch((err) => {
        ...
        // Always sync policies from the backend — even an empty array means all
        // policies were deleted and the mock seed data should be cleared.
        if (backendPolicies) {
          dispatch({ type: 'SET_POLICIES', payload: backendPolicies });
        }
