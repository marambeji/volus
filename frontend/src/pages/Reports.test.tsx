...
beforeEach(() => {
  ...
  mockedGetLeaveTypes.mockResolvedValue([
    { id: 'lt-1', key: 'annual', label: 'Annual Leave', trackingMode: 'AVAILABLE_BALANCE', color: '#000000', displayOrder: 1 },
  ]);
...
describe('Reports page — Executive HR PDF Report integration', () => {
  it('fetches complete datasets and renders team report dashboard', async () => {
    ...
    expect(screen.getByText('Smart PDF Report')).toBeInTheDocument();
    expect(screen.getByText('Export CSV')).toBeInTheDocument();
  });

  it('renders official corporate PDF report modal trigger option', async () => {
    await act(async () => {
      render(<Reports />);
    });

    expect(screen.getByText('Smart PDF Report')).toBeInTheDocument();
    expect(screen.getByText('AI Manager Analytics')).toBeInTheDocument();
  });
});
