...
const sections = [
  {
    ...
    items: [
      ...
      { label: 'Full Calendar', path: '/full-calendar', icon: Calendar },
    ]
  },
  {
    title: 'EMPLOYEES',
  ...
  },
  {
    title: 'MANAGEMENT',
    items: [
      { label: 'Approval Dashboard', path: '/approval-dashboard', icon: CheckSquare },
    ]
  }
];

export default function Navbar({ isOpen, onClose, onLogout: _onLogout, onSwitchRole }: NavbarProps) {
  return (
    <>
      ...
      >
        ...
        <div className="p-4 border-t border-[#1e306e]/20 flex flex-col gap-3">
          <button
            onClick={() => onSwitchRole ? onSwitchRole('admin') : undefined}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold rounded-xl transition-colors cursor-pointer"
          >
            Switch to HR Admin
          </button>
          <div className="text-[10px] text-slate-500 font-bold tracking-wider text-center">
            © 2026 NOVELUS
          </div>
        </div>
      </aside>
