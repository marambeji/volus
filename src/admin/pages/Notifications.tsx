...
export default function Notifications() {
  ...
  const filteredLogs = search.trim()
    ? logs.filter(n => {
        const title   = (ACTION_TITLES[n.actionType] ?? n.actionType.replace(/_/g, ' ')).toLowerCase();
        const message = (n.description ?? `${n.actorName} — ${title}`).toLowerCase();
        const actor   = (n.actorName ?? '').toLowerCase();
        const q       = search.toLowerCase();
        return title.includes(q) || message.includes(q) || actor.includes(q);
      })
    : logs;
