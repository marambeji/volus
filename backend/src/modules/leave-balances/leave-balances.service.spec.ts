...
describe('LeaveBalancesService Unit Tests', () => {
  ...
  describe('runAccruals', () => {
    const makeEmployee = (rule: Record<string, unknown>) => ({
      id: 'emp1',
      policyAssignments: [
        {
          isActive: true,
            ...
            rules: [
              {
                ...
                maxBalanceCap: 10,
                leaveType: { id: 'lt1', isActive: true },
                ...rule,
              },
            ],
          },
    ...
    it('creates a clamped ACCRUAL ledger entry for a firing MONTHLY rule', async () => {
      employeeRepo.find.mockResolvedValue([makeEmployee({})]);
      ledgerRepo.findOne.mockResolvedValue(null);

      const mockEm = {
        findOne: jest.fn().mockResolvedValue({ id: 'b1', availableBalance: 9 }),
      ...
      const result = await service.runAccruals(6, 2026);

      expect(result.accrualEntriesCreated).toBe(1);
      expect(result.processedEmployees).toBe(1);
      expect(mockEm.create).toHaveBeenCalledWith(
        LeaveLedgerEntry,
        expect.objectContaining({
          signedAmount: 1, // clamped: 9 + 2 -> capped at 10
          resultingBalance: 10,
    ...
    it('skips a YEARLY rule outside January', async () => {
      employeeRepo.find.mockResolvedValue([
        makeEmployee({ accrualInterval: AccrualInterval.YEARLY }),
      ]);
      ledgerRepo.findOne.mockResolvedValue(null);

      const result = await service.runAccruals(6, 2026);

      expect(result.accrualEntriesCreated).toBe(0);
      expect(dataSource.transaction).not.toHaveBeenCalled();
    });
  });
});
