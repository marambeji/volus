...
describe('EmployeesService Unit Tests', () => {
  ...
  it('should deactivate the active policy assignment and delete balances without ledger history when policyId is explicitly cleared', async () => {
    ...
    employeeRepo.findOne.mockResolvedValue({
      ...
      status: EmployeeStatus.ACTIVE,
      countryId: 'c1',
      policyAssignments: [activeAssignment],
    });

    const emSave = jest.fn((obj: Record<string, unknown>) => Promise.resolve(obj));
    const emFind = jest
      .fn()
      .mockResolvedValue([{ id: 'bal-unused' }, { id: 'bal-with-history' }]);
    const emCount = jest
      .fn()
      .mockResolvedValueOnce(0) // bal-unused: no ledger entries -> deleted
      .mockResolvedValueOnce(2); // bal-with-history: has ledger entries -> kept
    const emRemove = jest.fn().mockResolvedValue(undefined);
    ...
    await service.update('e1', { policyId: null });

    expect(activeAssignment.isActive).toBe(false);
